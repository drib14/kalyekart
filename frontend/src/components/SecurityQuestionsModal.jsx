...
const SecurityQuestionsModal = () => {
	...
	const { mutate: setSecurityQuestions, isLoading, isPending } = useMutation({
		mutationFn: (data) => axios.post("/auth/set-security-questions", data),
		onSuccess: () => {
	...
	// Don’t render if no user or already set
		...
		<div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
			<div className="bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-sm sm:max-w-md md:max-w-lg max-h-[90vh] overflow-y-auto">
				...
				<p className="text-gray-400 text-sm mb-2">
					This is a required step to ensure you can recover your account if you
					forget your password.
				</p>
				<p className="text-xs sm:text-sm text-yellow-400 mb-4">
					Warning: Please remember your answers. You will need them to recover
					your password.
				</p>
              ...
              focus:ring-emerald-500 transition duration-150 ease-in-out disabled:opacity-50"
					>
						{isLoading || isPending ? <LoadingSpinner /> : "Save Security Questions"}
					</button>
				</form>
			</div>
		</div>
